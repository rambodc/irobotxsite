...
Pull requests retain the required Hosting, Functions and Rules checks, but skip unrelated test suites using `scripts/change-scope.mjs`. Frontend changes run frontend checks; backend changes run backend and authorization tests; workflow changes validate all suites. Required checks still report a result even when their expensive steps are skipped.
